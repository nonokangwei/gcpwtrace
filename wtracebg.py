...
def run_wtrace(country_code, dest_url, bq_client, table_id, agent_ip, run_info, pid, agent_num, loop):
    wtrace_cmd = '/google/data/ro/teams/internetto/wtrace --nowait --ip_version=4 --agent={} {}'.format(agent_ip['address'], dest_url)
    wtrace_cmd += ' | grep "Response code\|Time elapsed\|The trace ran on\|rtt min/avg/max\|Using IP address"'
    try:
...
def start_wtrace_thread_pool(max_worker, country_code, dest_url, bq_client, table_id, num_loop, agent_list, run_info, interval, max_hours):
...
def main():
    ...
    # Init tableid
    latency_table_id = "lxd-project.wtrace.run"         #<==== modify to your table
    agent_table_id = "lxd-project.wtrace.agent_info"    #<==== modify to your table
    ...
    start_wtrace_thread_pool(max_worker, country_code, dest_url, client, latency_table_id, num_loop, agent_list, run_info, interval, max_hours)
    client.close()
